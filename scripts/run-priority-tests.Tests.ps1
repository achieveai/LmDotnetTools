...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10813 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10813 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 414), @("P1", 8164), @("P2", 2088), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1002 "Rows absent from the frozen capture"
