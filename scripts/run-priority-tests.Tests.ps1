...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10805 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10805 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 414), @("P1", 8159), @("P2", 2085), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 994 "Rows absent from the frozen capture"
