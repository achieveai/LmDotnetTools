...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9533 -and $repositoryPlan.declarationSummary.reviewed -eq 9533) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 277), @("P1", 7259), @("P2", 1870), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 476) "Manifest integration must preserve declarations absent from the frozen capture."
