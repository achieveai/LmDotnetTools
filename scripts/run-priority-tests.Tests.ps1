...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10909 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10909 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 418), @("P1", 8231), @("P2", 2110), @("P3", 150))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1102 "Rows absent from the frozen capture"
