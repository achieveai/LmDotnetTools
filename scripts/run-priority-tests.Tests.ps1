...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10840 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10840 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 416), @("P1", 8183), @("P2", 2092), @("P3", 149))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1029 "Rows absent from the frozen capture"
