...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10494 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10494 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 347), @("P1", 7974), @("P2", 2027), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 676 "Rows absent from the frozen capture"
