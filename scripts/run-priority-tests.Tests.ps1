...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10548 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10548 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 8013), @("P2", 2041), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 731 "Rows absent from the frozen capture"
