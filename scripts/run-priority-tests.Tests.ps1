...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10787 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10787 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 414), @("P1", 8144), @("P2", 2082), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 974 "Rows absent from the frozen capture"
