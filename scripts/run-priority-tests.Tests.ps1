...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10761 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10761 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 406), @("P1", 8130), @("P2", 2078), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 948 "Rows absent from the frozen capture"
