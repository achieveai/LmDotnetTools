...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9956 -and $repositoryPlan.declarationSummary.reviewed -eq 9956) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 290), @("P1", 7643), @("P2", 1896), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 240) "Manifest integration must preserve declarations absent from the frozen capture."
