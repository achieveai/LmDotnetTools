...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10486 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10486 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 347), @("P1", 7967), @("P2", 2026), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 668 "Rows absent from the frozen capture"
