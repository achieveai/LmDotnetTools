...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10552 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10552 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 8014), @("P2", 2043), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 735 "Rows absent from the frozen capture"
