...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9766 -and $repositoryPlan.declarationSummary.reviewed -eq 9766) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 274), @("P1", 7518), @("P2", 1847), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "measured" }).Count -eq 9624) "Manifest integration must preserve every measured coverage classification."
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 38) "Manifest integration must preserve declarations absent from the frozen capture."
