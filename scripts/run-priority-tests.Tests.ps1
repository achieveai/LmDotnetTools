...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9823 -and $repositoryPlan.declarationSummary.reviewed -eq 9823) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 278), @("P1", 7555), @("P2", 1863), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 102) "Manifest integration must preserve declarations absent from the frozen capture."
