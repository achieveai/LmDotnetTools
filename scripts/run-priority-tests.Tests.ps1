...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9970 -and $repositoryPlan.declarationSummary.reviewed -eq 9970) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 291), @("P1", 7655), @("P2", 1897), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 254) "Manifest integration must preserve declarations absent from the frozen capture."
