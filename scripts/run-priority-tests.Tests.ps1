...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9869 -and $repositoryPlan.declarationSummary.reviewed -eq 9869) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 286), @("P1", 7579), @("P2", 1877), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "measured" }).Count -eq 9617) "Manifest integration must preserve every measured coverage classification whose declaration still exists."
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 148) "Manifest integration must preserve declarations absent from the frozen capture."
