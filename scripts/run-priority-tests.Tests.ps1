...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10470 -and $repositoryPlan.declarationSummary.reviewed -eq 10470) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 652) "Manifest integration must preserve declarations absent from the frozen capture."
