...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10265 -and $repositoryPlan.declarationSummary.reviewed -eq 10265) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 347), @("P1", 7836), @("P2", 1951), @("P3", 131))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "no-coverage-capture" }).Count -eq 205) "Manifest integration must preserve approved projects without coverage capture."
