...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10178 -and $repositoryPlan.declarationSummary.reviewed -eq 10178) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 305), @("P1", 7783), @("P2", 1957), @("P3", 133))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "no-coverage-capture" }).Count -eq 106) "Manifest integration must preserve approved projects without coverage capture."
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 462) "Manifest integration must preserve declarations absent from the frozen capture."
