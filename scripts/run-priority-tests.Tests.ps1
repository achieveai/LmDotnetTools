...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10821 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10821 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 416), @("P1", 8169), @("P2", 2087), @("P3", 149))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "measured" }).Count 9603 "Measured coverage classifications"
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1010 "Rows absent from the frozen capture"
