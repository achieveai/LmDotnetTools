...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9941 -and $repositoryPlan.declarationSummary.reviewed -eq 9941) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 290), @("P1", 7633), @("P2", 1891), @("P3", 127))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 225) "Manifest integration must preserve declarations absent from the frozen capture."
