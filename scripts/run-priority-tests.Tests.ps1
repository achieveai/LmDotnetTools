...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10306 -and $repositoryPlan.declarationSummary.reviewed -eq 10306) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 7863), @("P2", 1960), @("P3", 135))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 489) "Manifest integration must preserve declarations absent from the frozen capture."
