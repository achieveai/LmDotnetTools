...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10532 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10532 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 8004), @("P2", 2034), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 715 "Rows absent from the frozen capture"
