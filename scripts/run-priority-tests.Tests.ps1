...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10833 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10833 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 416), @("P1", 8177), @("P2", 2091), @("P3", 149))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1022 "Rows absent from the frozen capture"
