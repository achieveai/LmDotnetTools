...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10160 -and $repositoryPlan.declarationSummary.reviewed -eq 10160) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 303), @("P1", 7776), @("P2", 1951), @("P3", 130))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 445) "Manifest integration must preserve declarations absent from the frozen capture."
