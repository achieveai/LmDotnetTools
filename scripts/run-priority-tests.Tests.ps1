...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10508 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10508 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 347), @("P1", 7981), @("P2", 2034), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 690 "Rows absent from the frozen capture"
