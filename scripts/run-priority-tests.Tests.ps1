...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10551 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10551 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 8014), @("P2", 2043), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 734 "Rows absent from the frozen capture"
