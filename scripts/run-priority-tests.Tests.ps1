...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10531 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10531 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 8003), @("P2", 2034), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 714 "Rows absent from the frozen capture"
