...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10910 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10910 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 418), @("P1", 8231), @("P2", 2111), @("P3", 150))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "measured" }).Count 9599 "Measured coverage classifications"
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1103 "Rows absent from the frozen capture"
