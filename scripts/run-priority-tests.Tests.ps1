...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10867 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10867 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 416), @("P1", 8203), @("P2", 2099), @("P3", 149))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 1058 "Rows absent from the frozen capture"
