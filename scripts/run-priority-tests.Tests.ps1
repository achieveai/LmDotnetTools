...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10251 -and $repositoryPlan.declarationSummary.reviewed -eq 10251) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 344), @("P1", 7825), @("P2", 1951), @("P3", 131))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "no-coverage-capture" }).Count -eq 194) "Manifest integration must preserve approved projects without coverage capture."
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 447) "Manifest integration must preserve declarations absent from the frozen capture."
