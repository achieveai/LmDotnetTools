...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10300 -and $repositoryPlan.declarationSummary.reviewed -eq 10300) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 7858), @("P2", 1959), @("P3", 135))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "measured" }).Count -eq 9609) "Manifest integration must preserve every measured coverage classification whose declaration still exists."
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "no-coverage-capture" }).Count -eq 208) "Manifest integration must preserve approved projects without coverage capture."
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 483) "Manifest integration must preserve declarations absent from the frozen capture."
