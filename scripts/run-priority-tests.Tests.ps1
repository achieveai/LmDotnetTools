...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10172 -and $repositoryPlan.declarationSummary.reviewed -eq 10172) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 303), @("P1", 7781), @("P2", 1956), @("P3", 132))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 457) "Manifest integration must preserve declarations absent from the frozen capture."
