...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10804 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10804 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 414), @("P1", 8158), @("P2", 2085), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 993 "Rows absent from the frozen capture"
