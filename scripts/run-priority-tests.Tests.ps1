...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10501 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10501 "Reviewed policy rows"
    ...
    foreach ($tier in @(@("P0", 347), @("P1", 7977), @("P2", 2031), @("P3", 146))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "measured" }).Count 9610 "Measured coverage classifications"
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 683 "Rows absent from the frozen capture"
