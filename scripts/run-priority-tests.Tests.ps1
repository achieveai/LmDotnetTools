...
try {
    ...
    foreach ($tier in @(@("P0", 348), @("P1", 8031), @("P2", 2043), @("P3", 147))) {
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 752 "Rows absent from the frozen capture"
