...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 10175 -and $repositoryPlan.declarationSummary.reviewed -eq 10175) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 303), @("P1", 7783), @("P2", 1957), @("P3", 132))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 460) "Manifest integration must preserve declarations absent from the frozen capture."
