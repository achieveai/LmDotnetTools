...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9978 -and $repositoryPlan.declarationSummary.reviewed -eq 9978) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 290), @("P1", 7659), @("P2", 1900), @("P3", 129))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 261) "Manifest integration must preserve declarations absent from the frozen capture."
