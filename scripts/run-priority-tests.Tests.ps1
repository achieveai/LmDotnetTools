...
try {
    ...
    Assert-True ($repositoryPlan.declarationSummary.known -eq 9985 -and $repositoryPlan.declarationSummary.reviewed -eq 9985) "Every known .NET/script declaration must have exactly one reviewed policy row."
    ...
    foreach ($tier in @(@("P0", 291), @("P1", 7664), @("P2", 1901), @("P3", 129))) {
    ...
    Assert-True (@($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count -eq 268) "Manifest integration must preserve declarations absent from the frozen capture."
