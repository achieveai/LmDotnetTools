...
try {
    ...
    Assert-Count $repositoryPlan.declarationSummary.known 10470 "Known .NET/script declarations"
    Assert-Count $repositoryPlan.declarationSummary.reviewed 10470 "Reviewed policy rows"
    ...
    Assert-Count @($manifestRows | Where-Object { $_.kind -eq "test-declaration" -and $_.coverageEvidence -eq "not-in-capture" }).Count 652 "Rows absent from the frozen capture"
