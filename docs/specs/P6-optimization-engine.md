...
| Capability denial | `OperationPolicy.Decide` (`samples/CodeReviewDaemon.Sample/Workspace/OperationPolicy.cs:121`) — host-side, and genuinely enforcing. **No tool-side counterpart:** `ScopedToolFilter` was deleted (#490) and `ReadOnlyToolFilter` (`samples/CodeReviewDaemon.Sample/Agents/ReviewToolContext.cs:33`) has no production caller, so nothing in this process bounds the hosted agent's tool set. | code for the host-side half only. **Partial.** The agent's writes are bounded by the commit gate (`stagePaths`) + `SlotHygiene`, not by capability denial. |
| Deterministic caps | input-side only: `MaxExistingCommentsRendered = 400` (`samples/CodeReviewDaemon.Sample/Orchestration/DaemonReviewStageExecutor.cs:2595`), `MaxExistingCommentsChars = 40_000` (`:2610`) — one **approximate** block budget shared by the past-reviews and new-since-last-review sections, claimed by the newer one FIRST against the whole block, with `NewCommentsReservedChars` (`:2634`) capping the past section at the other half; both numbers are stated to the model in the rendered block; `ExistingCommentBody.MaxChars = 2000` per comment (`samples/CodeReviewDaemon.Sample/Orchestration/IReviewCommentPublisher.cs:101`); `MaxKnowledgeEntries = 24` (`DaemonReviewStageExecutor.cs:1674`), `MaxKnowledgeDigestChars` (`:1677`), `UntrustedTranscriptText` caps (`samples/CodeReviewDaemon.Sample/Orchestration/ReviewNotesArtifactBuilder.cs:38`, `:41`) | code. But **no cap, dedupe, or path/line filter on findings.** |
| | Why the block budget is *approximate*: `ExistingCommentsBudget.IsSpent` is tested **before** a comment is charged, so a section can overshoot by its last thread header plus one last comment (body ≤ 2000 + the `(author, date)` wrapper ≈ 2,050 chars). Only ONE section can overshoot: the new section claims first against the whole block, and the past section's allowance is then whatever the new one left, floored at zero — so a new-section overshoot leaves the past section nothing to spend rather than adding to it. Worst case ≈ **42,050** chars, not 40,000. (Under the previous past-first order the past section's overshoot was floored at zero *against a fixed reservation the new section got regardless*, so the two overshoots were independent and the worst case was ≈ 44,100.) The tests pin the order of magnitude deliberately, not the exact bound. | |
...
| S3 | A bot comment was **deleted** | **Unavailable on both.** Absence from a later listing cannot prove deletion: the listing is capped at `MaxExistingCommentsRendered = 400` (`samples/CodeReviewDaemon.Sample/Orchestration/DaemonReviewStageExecutor.cs:2595`) and that cap is a *render* bound, not a retrieval bound. Nothing persists the set of comment ids the daemon published in a round | — |
