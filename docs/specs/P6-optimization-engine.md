...
| Deterministic caps | input-side only: `MaxExistingCommentsListed = 120` (`samples/CodeReviewDaemon.Sample/Orchestration/DaemonReviewStageExecutor.cs:2312`), `MaxExistingCommentsChars = 40_000` (`:2327`) — one **approximate** block budget shared by the past-reviews and new-since-last-review sections, with `NewCommentsReservedChars` (`:2340`) reserving half as a floor for the newer one; `ExistingCommentBody.MaxChars = 2000` per comment (`samples/CodeReviewDaemon.Sample/Orchestration/IReviewCommentPublisher.cs:101`); `MaxKnowledgeEntries = 24` (`DaemonReviewStageExecutor.cs:1674`), `MaxKnowledgeDigestChars` (`:1677`), `UntrustedTranscriptText` caps (`samples/CodeReviewDaemon.Sample/Orchestration/ReviewNotesArtifactBuilder.cs:38`, `:41`) | code. But **no cap, dedupe, or path/line filter on findings.** |
| | Why the block budget is *approximate*: `ExistingCommentsBudget.IsSpent` is tested **before** a comment is charged, so each of the two sections can overshoot by its last thread header plus one last comment (body ≤ 2000 + the `(author, date)` wrapper ≈ 2,050 chars). The past section's overshoot is floored at zero rather than deducted from the new section's carry-forward, so the two overshoots are independent: worst case ≈ **44,100** chars, not 40,000. The tests pin the order of magnitude deliberately, not the exact bound. | |
