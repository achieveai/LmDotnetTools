...
> **R3 follow-up (verified during execution, 2026-07-05):** the gateway does NOT deny the agent's Bash egress even with an explicit `{"network":{"rules":[{"action":"deny","hosts":["*"],…}]}}` policy — the agent still reached `example.com` (`rc=0`). So **egress-deny is currently unachievable via the gateway.** Per the fallback above, the scoped **Write/Bash reviewer is gated to reviews where exfil is not a confidentiality breach** — i.e. the reviewed repo is public (reuse `ReviewRun.IsTargetRepoPublic` / the confidentiality gate). For a private/unknown-trust target the reviewer stays **read-only** (no Write/Edit/Bash), exactly as before this layer. Lifting this restriction requires a gateway egress-deny capability (tracked as a gateway follow-up).
