...
> **Layer 3 removed (2026-08-27, PR #497 / issue #490):** `ScopedToolFilter` has been **deleted**. When the daemon went mandatory-S2S the in-process registry-building path was retired, so nothing applied the wrapper — the review runs in a conversation the LmStreaming review host owns and the daemon builds no tool registry for it. Layer 3 above is therefore a record of the original decision, not a control that stands today. The surviving enforcement is (1) the commit gate — `DaemonReviewStageExecutor.CommitPooledNotesAsync` commits with `stagePaths: [lease.NotesRelPath]` — plus (2) no write credential, and `SlotHygiene`'s strip + clean-on-entry, which erases the rest of the slot before reuse. **The R1 finding is unchanged and still load-bearing:** a per-path read-only mount is unavailable, so the sandbox mount is NOT the write-scope boundary and `repos/<Repo>` stays writable — see the "Consequence for `repos/<Repo>`" note below.
