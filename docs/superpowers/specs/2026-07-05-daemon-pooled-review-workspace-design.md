...
> **Layer 3 removed (2026-08-27, PR #497 / issue #490):** `ScopedToolFilter` has been **deleted**. When the daemon went mandatory-S2S the in-process registry-building path was retired, so nothing applied the wrapper — the review runs in a conversation the LmStreaming review host owns and the daemon builds no tool registry for it. Layer 3 above is therefore a record of the original decision, not a control that stands today. The surviving enforcement is (1) the commit gate — `DaemonReviewStageExecutor.CommitPooledNotesAsync` commits with `stagePaths: [lease.NotesRelPath]` — plus (2) no write credential, and `SlotHygiene`'s **clean-on-entry** (`ReviewSlotPreparer.PrepareAsync` → `EnsureCleanAsync`, `ReviewSlotPreparer.cs:233`, unconditional), which erases the rest of the slot on the next lease. **Do not cite the clean-on-EXIT strip as a control:** `SlotHygiene.StripAsync`'s only call site (`DaemonReviewStageExecutor.cs:3857`) is guarded by `if (!_options.UseS2SReviewAgent)`, and `Program.cs:289-294` refuses to boot with S2S off, so the strip is unreachable in production — as `StripAsync`'s own doc says, "a caller that cannot make that promise must not call this at all, and the S2S one does not". **The R1 finding is unchanged and still load-bearing:** a per-path read-only mount is unavailable, so the sandbox mount is NOT the write-scope boundary and `repos/<Repo>` stays writable — see the "Consequence for `repos/<Repo>`" note below.

> **R3 follow-up (verified during execution, 2026-07-05):** the gateway does NOT deny the agent's Bash egress even with an explicit `{"network":{"rules":[{"action":"deny","hosts":["*"],…}]}}` policy — the agent still reached `example.com` (`rc=0`). So **egress-deny is currently unachievable via the gateway.** **Decision (user, 2026-07-05): accept the residual risk** — the scoped Write/Edit/Bash reviewer is granted whenever `EnableReviewerWrites` is on, regardless of target visibility, relying on the untrusted-content framing + the commit gate (nothing persists) + the narrow threat model (exfil of code the reviewer is already reading). The residual is that a prompt-injected agent has open network egress; a gateway egress-deny capability remains a desirable follow-up but is not a blocker. (No `IsTargetRepoPublic` gating on the writable surface.)
