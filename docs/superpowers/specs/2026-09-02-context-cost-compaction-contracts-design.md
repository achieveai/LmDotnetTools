...
Rollback (`Active → RolledBack`): triggered when the provider still overflows after a reactive compaction, or by the kill switch. The row stays. The UI keeps the divider (history is truth) with a "rolled back" badge. The next request uses `LastKnownGoodCheckpointId` if one exists, else raw history. The kill switch clears the last known good, so after a kill it is always raw history (§8.4).
...
Kill = `Skipped(disabled)` for new decisions **and** `Active → RolledBack` for the next request in every owned loop (rows untouched). Re-enable does not re-activate automatically; the next policy pass may compact again. Kill does not fall back to the last known good: the active and every superseded checkpoint become `RolledBack(killed)` and the last known good is cleared, so no older checkpoint can come back.
