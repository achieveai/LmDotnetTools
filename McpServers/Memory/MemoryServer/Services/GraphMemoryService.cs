...
public class GraphMemoryService : IGraphMemoryService
{
    ...
    )
    {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Extracted {EntityCount} entities and {RelationshipCount} relationships",
                entities.Count(),
                relationships.Count()
            );
            ...
            {
                var isValid = await _decisionEngine.ValidateGraphUpdateAsync(
                    instruction,
                    sessionContext,
                    cancellationToken
                );
                if (!isValid)
                {
                    ...
                    _logger.LogWarning(
                        "RELATIONSHIP VALIDATION FAILED: {RelationshipInfo} | Operation: {Operation} | Reasoning: {Reasoning} | Confidence: {InstructionConfidence:F2}",
                        relationshipInfo,
            ...
            summary.ProcessedEntities = [.. entities.Select(e => e.Name).Distinct()];
            summary.ProcessedRelationshipTypes = [.. relationships.Select(r => r.RelationshipType).Distinct()];

            stopwatch.Stop();
            summary.ProcessingTimeMs = stopwatch.ElapsedMilliseconds;

            _logger.LogInformation(
                "Processed memory {MemoryId}: {EntitiesAdded} entities added, {RelationshipsAdded} relationships added",
                memory.Id,
    ...
    )
    {
        ...
        try
        {
            ...
            _ = await Task.WhenAll(traditionalTask, graphTask);

            results.TraditionalResults = await traditionalTask;
            results.GraphResults = await graphTask;

            // Combine and rank results
            results.CombinedResults = CombineSearchResults(
            ...
            _logger.LogDebug(
                "Hybrid search completed: {TraditionalCount} traditional, {GraphCount} graph, {CombinedCount} combined results",
                results.TraditionalResults.Count,
                results.GraphResults.Count,
                results.CombinedResults.Count
            );
    ...
    )
    {
        ...
        {
            _logger.LogDebug(
                "Getting related entities for '{EntityName}' with max depth {MaxDepth}",
                entityName,
                maxDepth
            );

            // Get the starting entity
            var startEntity = await _graphRepository.GetEntityByNameAsync(
                entityName,
                sessionContext,
                cancellationToken
            );
            ...
            _logger.LogDebug(
                "Graph traversal completed: {EntityCount} entities, {RelationshipCount} relationships, max depth {MaxDepth}",
                result.AllEntities.Count,
                result.AllRelationships.Count,
    ...
    )
    {
        ...
        {
            _logger.LogInformation(
                "Starting graph rebuild for session {UserId}/{AgentId}/{RunId}",
            ...
            _logger.LogInformation(
                "Graph rebuild completed: {EntitiesCreated} entities, {RelationshipsCreated} relationships created in {ElapsedMs}ms",
                summary.EntitiesCreated,
                summary.RelationshipsCreated,
                summary.RebuildTimeMs
            );
    ...
    )
    {
        ...
        try
        {
                ...
                if (!entityNames.Contains(relationship.Source))
                {
                    ...
                    result.Errors.Add(
                        new GraphValidationError
                        {
                            ...
                            Description =
                                $"Relationship {relationship.Id} references non-existent source entity '{relationship.Source}'",
                ...
                if (!entityNames.Contains(relationship.Target))
                {
                    ...
                    result.Errors.Add(
                        new GraphValidationError
                        {
                            Type = GraphValidationErrorType.BrokenReference,
                            Description =
                                $"Relationship {relationship.Id} references non-existent target entity '{relationship.Target}'",
                            RelationshipId = relationship.Id,
                            Severity = ValidationSeverity.High,
                        }
                    );
            ...
            foreach (var relationship in relationships)
            {
                _ = connectedEntities.Add(relationship.Source);
                _ = connectedEntities.Add(relationship.Target);
            }
            ...
            _logger.LogDebug(
                "Graph validation completed: {IsValid}, {ErrorCount} errors, {WarningCount} warnings",
                result.IsValid,
                result.Errors.Count,
    ...
    )
    {
        ...
        {
            _logger.LogDebug(
                "EXECUTION START: Operation={Operation}, HasEntity={HasEntity}, HasRelationship={HasRelationship}",
                instruction.Operation,
            ...
            switch (instruction.Operation)
            {
                case GraphDecisionOperation.ADD:
                    if (instruction.EntityData != null)
                    {
                        ...
                        _ = await _graphRepository.AddEntityAsync(
                    ...
                    else if (instruction.RelationshipData != null)
                    {
                        ...
                        _ = await _graphRepository.AddRelationshipAsync(
                ...
                case GraphDecisionOperation.UPDATE:
                    if (instruction.EntityData != null)
                    {
                        _ = await _graphRepository.UpdateEntityAsync(
                            instruction.EntityData,
                            instruction.SessionContext,
                            cancellationToken
                        );
                    ...
                    {
                        _ = await _graphRepository.UpdateRelationshipAsync(
                            instruction.RelationshipData,
                            instruction.SessionContext,
                            cancellationToken
                        );
                ...
                case GraphDecisionOperation.DELETE:
                    if (instruction.EntityData != null)
                    {
                        _ = await _graphRepository.DeleteEntityAsync(
                            instruction.EntityData.Id,
                            instruction.SessionContext,
                            cancellationToken
                        );
                    }
                    ...
                    {
                        _ = await _graphRepository.DeleteRelationshipAsync(
                            instruction.RelationshipData.Id,
                            instruction.SessionContext,
                            cancellationToken
                        );
                    }
        ...
        {
            var instructionInfo =
                instruction.EntityData != null ? $"Entity: {instruction.EntityData.Name}"
                : instruction.RelationshipData != null
            ...
            _logger.LogError(
                ex,
                "EXECUTION FAILED: {InstructionInfo} | Operation: {Operation} | Reasoning: {Reasoning} | Error: {ErrorMessage}",
                instructionInfo,
                instruction.Operation,
                instruction.Reasoning,
                ex.Message
    ...
    )
    {
        try
        {
            ...
            {
                var traversalResult = await GetRelatedEntitiesAsync(
                    entity.Name,
                    sessionContext,
                    2,
                    cancellationToken: cancellationToken
                ...
                foreach (var relatedEntity in traversalResult.AllEntities.Take(DefaultGraphSearchLimit))
                {
                    if (relatedEntity.SourceMemoryIds != null)
                    {
                        foreach (var memoryId in relatedEntity.SourceMemoryIds)
                        {
                            try
                            {
                                var memory = await _memoryRepository.GetByIdAsync(
                                    memoryId,
                                    sessionContext,
                                    cancellationToken
                                );
                            ...
                            catch (Exception ex)
                            {
                                _logger.LogWarning(
                                    ex,
                                    "Error retrieving memory {MemoryId} during graph search",
                                    memoryId
                                );
                            }
                        }
            ...
            return [.. memories.Take(maxResults)];
        }
        catch (Exception ex)
        {
    ...
    )
    {
        ...
        for (var i = 0; i < traditionalResults.Count; i++)
        {
            ...
            _ = processedMemoryIds.Add(memory.Id);
        }
        ...
        return [.. combinedResults.OrderByDescending(r => r.CombinedScore).Take(maxResults)];
    }
    ...
    private async Task ClearGraphDataAsync(SessionContext sessionContext, CancellationToken cancellationToken)
    {
        try
        {
            ...
            foreach (var relationship in relationships)
            {
                _ = await _graphRepository.DeleteRelationshipAsync(relationship.Id, sessionContext, cancellationToken);
            }

            // Delete all entities
            foreach (var entity in entities)
            {
                _ = await _graphRepository.DeleteEntityAsync(entity.Id, sessionContext, cancellationToken);
            }

            _logger.LogDebug(
                "Cleared {EntityCount} entities and {RelationshipCount} relationships",
                entities.Count(),
                relationships.Count()
            );
        }
