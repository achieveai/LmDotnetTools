...
- **Sandbox**: New `AchieveAi.LmDotnetTools.Sandbox` package (`net8.0`/`net9.0`, no ASP.NET/provider dependencies) — a credential-scoped `SandboxClient` for the sandbox gateway's control plane: authenticated create/get/list/delete lifecycle, marketplace preview, and session discovery; recoverable, single-submission command execution (`ExecuteAsync`) with exact UTF-8 output reassembled beyond gateway `exec` truncation and 24h same-operation-id idempotency; and exact, integrity-verified UTF-8 file transfer (`ReadTextFileAsync`/`WriteTextFileAsync`/`ListDirectoryAsync`). Ships with owned or borrowed `HttpClient` transport, stable `SandboxErrorKind`/`SandboxException` error classification, secret-redacted `SandboxClientOptions`, and an `AUTH_ENFORCE=true` live-gateway contract CI job. (#187, #188, #189, #190)
