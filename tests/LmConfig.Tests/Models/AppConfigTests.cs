...
public class AppConfigTests
{
  ...
  public void ProviderConnectionInfo_Validate_CatchesInvalidConfiguration()
  {
    ...
    System.Diagnostics.Debug.WriteLine($"Validation result: IsValid={validation.IsValid}, Errors={validation.Errors.Count}");

    // Assert
    Assert.False(validation.IsValid);
    Assert.Contains("EndpointUrl", validation.Errors.First(e => e.Contains("EndpointUrl")));
    Assert.Contains("ApiKeyEnvironmentVariable", validation.Errors.First(e => e.Contains("ApiKeyEnvironmentVariable")));
  ...
  public void ProviderConnectionInfo_Validate_PassesForValidConfiguration()
  {
    ...
    try
    {
      ...
      System.Diagnostics.Debug.WriteLine($"Validation result: IsValid={validation.IsValid}, Errors={validation.Errors.Count}, Warnings={validation.Warnings.Count}");
  ...
  public void AppConfig_DeserializesFromJson_WithProviderRegistry()
  {
    ...
    // Act
    var appConfig = JsonSerializer.Deserialize<AppConfig>(json, new JsonSerializerOptions
    {
      PropertyNameCaseInsensitive = true
    });

    System.Diagnostics.Debug.WriteLine($"Deserialized AppConfig: Models={appConfig?.Models.Count}, ProviderRegistry={appConfig?.ProviderRegistry?.Count}");
