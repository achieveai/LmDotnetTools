...
public class LoggingSystemIntegrationTests
{
    ...
    public async Task FunctionCallMiddleware_WithLogger_LogsCorrectly()
    {
        ...
        mockAgent.Setup(x => x.GenerateReplyAsync(It.IsAny<IEnumerable<IMessage>>(), It.IsAny<GenerateReplyOptions>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<IMessage> { new TextMessage { Text = "Response", Role = Role.Assistant } });
    ...
    public async Task FunctionCallMiddleware_WithToolCall_LogsFunctionExecution()
    {
        ...
        var middleware = new FunctionCallMiddleware(functions, functionMap, "logging-test-middleware", mockLogger.Object);
    ...
    public async Task FunctionCallMiddleware_WithDisabledLogging_HasMinimalOverhead()
    {
        ...
        Assert.True(timePerIteration < 50.0, // Less than 50ms per iteration
    ...
    public void StructuredLogging_WithComplexParameters_WorksCorrectly()
    {
        ...
        mockLogger.Object.LogInformation(LogEventIds.AgentRequestInitiated,
            "Request initiated: RequestId={RequestId}, Model={ModelId}, Params={Parameters}, Meta={Metadata}",
            complexData.RequestId, complexData.ModelId, complexData.Parameters, complexData.Metadata);
