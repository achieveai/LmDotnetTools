...
public sealed class SandboxWorkspaceGatewayE2ETests : LoggingTestBase
{
    ...
    public async Task Workspace_agent_runs_sandbox_tool_through_gateway_and_writes_to_host()
    {
        ...
        toolNames
            .Should()
            .Contain("Bash", "the gateway's Bash tool is folded into the registry as 'Bash' (omitServerPrefix: true)");

        // (2) Round-trip through the gateway: executing Bash returned the marker it just wrote.
        string.Concat(toolResults)
            .Should()
            .Contain(marker, "the sandbox Bash tool executed echo+cat through the gateway and returned the marker");
        ...
        var session = await factory
            .Services.GetRequiredService<SandboxSessionRegistry>()
        ...
        if (prereq.SpawnMode)
        {
            ...
            Logger.LogInformation(
                "Host file content length {Length}; contains marker={ContainsMarker}",
                hostContent.Length,
    ...
    public async Task Workspace_agent_runs_core_sandbox_tool_chain_through_gateway()
    {
        ...
        Logger.LogInformation(
            "Connecting WebSocket thread {ThreadId} in mode {ModeId}",
            threadId,
            SystemChatModes.WorkspaceAgentModeId
        );
        var socket = await factory.ConnectWebSocketAsync(threadId, SystemChatModes.WorkspaceAgentModeId);
