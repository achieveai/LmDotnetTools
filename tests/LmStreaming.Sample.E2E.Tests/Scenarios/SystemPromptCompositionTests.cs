...
/// in <c>Program.cs</c> (~1200 lines, no unit-test seam), and that gap is exactly where this field spent
...
/// <c>E2EWebAppFactory</c> boots the production DI graph and replaces only <see cref="ITestAgentBuilder"/> —
/// so the call site ran in every scenario here. But <c>AppendCallerInstructions</c> is a no-op on a
/// null/blank appendix, and no test had ever set one, so deleting the call produced byte-identical
/// behavior everywhere. Measured on this base before this test existed: removing the <c>ComposeAsync</c>
/// call from <c>Program.cs</c> left this suite at 90/90 green. A call site that no test can distinguish
/// from its own deletion is not covered by the tests that happen to execute it.
/// </para>
/// <para>
...
public sealed class SystemPromptCompositionTests
{
        ...
        string providerMode)
    {
        ...
        // 3. PrependCurrentDate reached the model too. Same uncovered call site, same failure mode: it is
        //    applied inside the factory lambda and nothing else observes it end-to-end.
