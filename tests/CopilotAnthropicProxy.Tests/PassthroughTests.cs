...
public sealed class PassthroughTests
{
    ...
    public async Task Status_body_and_rate_limit_headers_pass_through_while_hop_by_hop_are_stripped()
    {
        ...
        await using var factory = new ProxyWebAppFactory(
            (req, ct) => Task.FromResult(TestUpstream.Json(upstreamBody, HttpStatusCode.TooManyRequests, headers))
        );
        using var client = factory.CreateClient();

        using var response = await client.PostAsync(
            "/v1/messages",
            new StringContent("{\"model\":\"x\",\"max_tokens\":5}", Encoding.UTF8, "application/json")
        );
        ...
        response
            .Headers.GetValues("anthropic-ratelimit-requests-remaining")
            .Should()
            .ContainSingle()
            .Which.Should()
            .Be("0");
        response
            .Headers.GetValues("anthropic-ratelimit-tokens-remaining")
            .Should()
            .ContainSingle()
            .Which.Should()
            .Be("42");
    ...
    {
        const string upstreamBody =
            "{\"type\":\"message\",\"unknown_future_field\":{\"a\":1},\"content\":[{\"type\":\"text\",\"text\":\"hi\"}]}";
    ...
    public async Task Upstream_401_passes_through_verbatim_distinct_from_local_auth_error()
    {
        ...
        await using var factory = new ProxyWebAppFactory(
            (req, ct) => Task.FromResult(TestUpstream.Json(upstreamBody, HttpStatusCode.Unauthorized))
        );
        using var client = factory.CreateClient();
    ...
    {
        await using var factory = new ProxyWebAppFactory(
            (req, ct) => Task.FromException<HttpResponseMessage>(new HttpRequestException("simulated connect failure"))
        );
        using var client = factory.CreateClient();
