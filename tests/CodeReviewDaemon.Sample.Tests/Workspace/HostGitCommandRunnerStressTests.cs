...
/// <summary>
/// #104 — <see cref="HostGitCommandRunner"/> must never report a command as having SUCCEEDED with output
/// it did not capture.
/// <para>
...
/// </para>
/// </summary>
public sealed class HostGitCommandRunnerStressTests
{
    ...
    /// confused under load.
    /// </summary>
    private const int LingeringGrandchildSeconds = 20;
    ...
    public async Task RunAsync_UnderConcurrency_NeverReportsExitZeroWithNoOutput()
    {
        ...
        succeeded.Should().BeGreaterThan(
            0, "the probe must actually run, or an 'no empty captures' result means nothing");
    ...
    /// <summary>
    /// The other half of #104, and the hazard the first fix INTRODUCED: draining to EOF after the child
    /// exits is only safe if it is bounded.
    /// <para>
    ...
    /// <c>sh -c 'sleep N &amp; echo …'</c> reproduces exactly that shape: <c>sh</c> exits immediately, the
    /// backgrounded child inherits stdout and holds it. Without a deadline the runner would block for the
    /// grandchild's whole lifetime, turning a silent truncation into a silent hang on a path the daemon
    /// runs constantly. (<c>gc --auto</c> looks like the obvious real-world case and is NOT one — it
    /// detaches with its stdio redirected to .git/gc.log, so it never inherits ours.)
    /// </para>
    /// </summary>
    [Fact]
    public async Task RunAsync_WhenAGrandchildHoldsThePipeOpen_FailsLoudlyRatherThanWaitingForIt()
    {
        ...
        var result = await runner.RunAsync(
            new SandboxCommand(["sh", "-c", $"sleep {LingeringGrandchildSeconds} & echo hello"]),
            CancellationToken.None);

        started.Stop();

        started.Elapsed.Should().BeLessThan(
            TimeSpan.FromSeconds(LingeringGrandchildSeconds),
        ...
        result.Succeeded.Should().BeFalse(
            "once a write handle outlives the child there is no way to know the capture is complete, and "
                + "'possibly truncated' reported as success is the defect this ticket is about");
        result.Stderr.Should().Contain(
            "incomplete", "the condition has to name itself, or it is just a different silence");
        result.Stdout.Should().Contain(
            "hello", "what was read is still returned as evidence — it is the completeness that is in doubt");
    }
    ...
    public async Task RunAsync_WhenNothingHoldsThePipe_DrainsImmediatelyAndSucceeds()
    {
        ...
        var result = await runner.RunAsync(
            new SandboxCommand(["sh", "-c", "echo hello"]), CancellationToken.None);
