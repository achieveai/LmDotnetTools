...
public class HostGitCommandRunnerTests : IDisposable
{
    ...
    public async Task RunAsync_kills_and_fails_a_command_that_goes_silent_past_the_idle_timeout()
    {
        ...
        var result = await runner.RunAsync(new SandboxCommand(_lab.SilentFor(TimeSpan.FromSeconds(30)), _dir), default);
        ...
        (DateTimeOffset.UtcNow - started)
            .Should()
            .BeLessThan(TimeSpan.FromSeconds(15), "it must be killed near the timeout, not waited out");
    }
    ...
    public async Task RunAsync_leaves_a_chatty_long_command_alone_because_the_bound_is_inactivity()
    {
        ...
        result
            .Succeeded.Should()
            .BeTrue("the command outlived the idle timeout twice over while never once going quiet");
        result.Stdout.Should().Contain("working");
    }

    /// <summary>
    /// The absolute ceiling is the backstop for the case inactivity cannot catch: a remote that dribbles
    /// bytes forever, staying "active" indefinitely without ever finishing.
    /// </summary>
    [Fact]
    public async Task RunAsync_enforces_the_absolute_ceiling_even_while_output_keeps_arriving()
    {
        var runner = Runner(idleTimeout: TimeSpan.FromSeconds(30), maxDuration: TimeSpan.FromMilliseconds(2500));

        // Long enough that the ceiling lands mid-run, and talkative enough that several lines have already
        // arrived by then — so this cannot pass as a disguised idle-timeout.
        var result = await runner.RunAsync(new SandboxCommand(_lab.ChattyFor(10), _dir), default);

        result.Succeeded.Should().BeFalse();
        result
            .Stderr.Should()
            .Contain("ceiling", "a ceiling stop and an idle stop have different causes and must read differently");
    }
    ...
    public async Task RunAsync_rethrows_on_caller_cancellation_rather_than_reporting_a_failed_run()
    {
        ...
        var argv = _lab.SilentFor(TimeSpan.FromSeconds(30));

        var act = () => runner.RunAsync(new SandboxCommand(argv, _dir), cts.Token);
    ...
    /// <summary>
    /// The child process must actually DIE on both exit paths. A cancelled <c>WaitForExitAsync</c> alone
    /// abandons the process: a shutdown mid-fetch leaves a 969,911-object fetch running detached, still
    /// holding the store. That was rare while only shutdown could cancel — but a timeout makes it reachable
    /// on every stuck operation, so shipping the bound without the kill would turn a rare leak into a routine
    /// one.
    /// </summary>
    [Theory]
    [InlineData(true)]
    ...
    public async Task RunAsync_kills_the_child_rather_than_abandoning_it(bool byCallerCancellation)
    {
        ...
        var argv = _lab.WriteMarkerAfter(TimeSpan.FromSeconds(4), marker);
        var runner = Runner(idleTimeout: TimeSpan.FromMilliseconds(300));
        ...
        if (byCallerCancellation)
        {
            ...
            try
            {
                _ = await runner.RunAsync(new SandboxCommand(argv, _dir), cts.Token);
            }
        ...
        {
            _ = await runner.RunAsync(new SandboxCommand(argv, _dir), default);
        }
    ...
    public async Task RunAsync_announces_remote_operations_and_stays_quiet_for_local_plumbing()
    {
        ...
        _ = await Runner(localLog)
            .RunAsync(new SandboxCommand(["git", "rev-parse", "--is-inside-work-tree"], _dir), default);

        remoteLog
            .MessagesAtLevel(LogLevel.Information)
            .Should()
            .Contain(
        ...
        localLog
            .MessagesAtLevel(LogLevel.Information)
            .Should()
            .BeEmpty("every rev-parse announcing itself would bury the one line that matters");
    }
    ...
    {
        HostGitCommandRunner
            .IsProgressLine(line)
            .Should()
            .Be(
                isProgress,
                "a filter that also ate 'fatal:' or 'curl 56' would hide exactly the lines a stalled fetch is "
                    + "diagnosed from"
            );
    }
    ...
    /// <summary>
    /// Only <c>update</c> fetches. Adding the flag to a subcommand that neither talks to a remote nor accepts
    /// it would turn a working command into a usage error.
    /// </summary>
    [Theory]
    [InlineData("sync")]
    [InlineData("status")]
    ...
    /// <summary>
    /// The ordinary case still inserts at the verb — the submodule special case must not have moved it for
    /// everything else.
    /// </summary>
    [Fact]
    ...
    /// <summary>
    /// An explicit choice by the caller wins. Appending a second <c>--progress</c>, or overriding a
    /// deliberate <c>--no-progress</c>, would both be the tool disagreeing with its own call site.
    /// </summary>
    [Theory]
    [InlineData("--progress")]
