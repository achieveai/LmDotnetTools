...
public sealed class OrphanedPackSweeperTests : IDisposable
{
    ...
    public void A_temp_pack_this_very_process_holds_open_is_never_deleted()
    {
        ...
        {
            // /proc is where the evidence lives; there is no equivalent to fall back to. Reported as a known
            // limitation rather than worked around — see OrphanedPackSweeper.OpenTempPackPaths.
            return;
        }
    ...
    public void A_killed_fetch_leaves_the_pack_directory_exactly_as_it_found_it()
    {
        ...
        TempPackCount(lab.Store).Should().Be(
            beforeCount,
            "the pack directory must be returned to its pre-fetch state");
    }
    ...
    public void A_temp_pack_a_LIVE_fetch_is_still_writing_is_never_deleted()
    {
        ...
        File.Exists(live.TempPack).Should().BeTrue(
            "a temp pack another process still holds open belongs to a live fetch, not to us");
        files.Should().Be(0);
        bytes.Should().Be(0);
    }
    ...
    public void A_real_temp_pack_becomes_sweepable_only_once_its_writer_dies()
    {
        ...
        new FileInfo(fetch.TempPack).Length.Should().BeGreaterThan(
            0,
    ...
    private static int TempPackCount(string repo) =>
        Directory.Exists(PackDirectory(repo))
            ? Directory.GetFiles(PackDirectory(repo), "tmp_pack_*").Length
            : 0;
    ...
    {
        /// <summary>Payload size. Large enough that the transfer lasts seconds — the freeze has to land
        /// while <c>index-pack</c> is mid-write, and the reaction window is sub-millisecond — small enough
        /// that three of these cost a couple of hundred MB on a machine whose disk already failed once.</summary>
        private const int BlobBytes = 6 * 1024 * 1024;
        private const int BlobCount = 4;

        private readonly string _root;

        private GitLab(string root, string origin, string store)
        {
            _root = root;
            Origin = origin;
            Store = store;
        ...
        public static GitLab Create(string rootDirectory)
        {
            ...
            return new GitLab(root, origin, store);
        }
                ...
                while (DateTime.UtcNow < deadline)
                {
                    ...
                    if (candidate is not null && new FileInfo(candidate).Length > 0)
                    {
                        ...
                        File.Exists(candidate).Should().BeTrue(
                            "the fetch finished before it could be frozen; the payload is too small to catch");
                        return frozen;
                    }
        ...
        public void Dispose()
        {
            ...
            {
                // Disk is the resource that failed here, so a leftover lab is worth reporting but not worth
                // failing a green test over. The temp root is cleaned by the fixture's own Dispose too.
            }
            catch (UnauthorizedAccessException)
            {
                // Same: a read-only temp pack git left 0444 can refuse removal on some platforms.
            }
        }
        ...
        private static void Git(string? workingDirectory, params string[] arguments)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"git {string.Join(' ', arguments)} failed with {process.ExitCode}: {process.StandardError.ReadToEnd()}");
            }
        }
        ...
        /// <summary>Every descendant pid of <paramref name="rootPid"/>, read from <c>/proc</c> rather than
        /// shelled out to <c>pgrep</c> so the fixture depends on nothing beyond the kernel.</summary>
        private static List<int> DescendantsOf(int rootPid)
        {
            ...
            foreach (var directory in Directory.GetDirectories("/proc"))
            {
                ...
                try
                {
                    ...
                    var tail = stat[(stat.LastIndexOf(')') + 1)..].Split(
                        ' ',
    ...
    /// <summary>A fetch held motionless mid-pack, so a test can ask what the sweeper does about a temp pack
    /// whose writer is demonstrably still alive and still holding it open.</summary>
    private sealed class FrozenFetch : IDisposable
    {
        ...
        /// <summary>Kills the frozen fetch the way the daemon's watchdog does, abandoning its pack. A stopped
        /// process must be continued first or SIGKILL is merely queued and the descriptor stays open — which
        /// would make the "abandoned" half of every paired test silently measure the "live" case instead.</summary>
