...
public sealed class S2SReviewAgentTests
{
    ...
    public async Task ExecuteRunAsync_rejoins_an_armed_input_instead_of_queueing_a_second_turn()
    {
        ...
        resumable.ArmTurnCheckpoint(SynthesisKey, "input-inflight", (inputId, _) => reAccepted.Add(inputId));
    ...
    public async Task ExecuteRunAsync_reports_a_newly_accepted_input_id_before_it_starts_polling()
    {
        var handler = new FakeHttpMessageHandler()
            ...
            .OnJson(
                HttpMethod.Post,
                "/messages",
                "{\"inputId\":\"input-minted\",\"idempotencyKeyHonored\":true,\"modelId\":\"gpt-5.6-sol\"}"
            )
            .OnJson(
                HttpMethod.Get,
                "/status",
        ...
        agent.UseModelForNextTurn("gpt-5.6-sol");
        _ = await DriveAsync(agent, "synthesize the final review");

        accepted.Should().Equal("input-minted");
        acceptedModelId
            .Should()
            .Be("gpt-5.6-sol", "the callback exposes the host acknowledgement, not request metadata");
    ...
    public async Task ExecuteRunAsync_arms_only_the_next_turn()
    {
        ...
        IResumableReviewTurn resumable = agent;
        resumable.ArmTurnCheckpoint(SynthesisKey, "input-inflight", (_, _) => { });
