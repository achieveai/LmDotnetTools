...
public sealed class CollectOnlyProviderWriteGateTests : LoggingTestBase
{
    ...
    /// <summary>
    /// The provider READS the review depends on, on a collect-only run. These are not decoration: the PR
    /// thread route is what lets the reviewer read the comments already on the PR, and the linked-work-item
    /// route is the answer to "what was this change asked to do". A gate that closed either would be a
    /// regression dressed as a safety net — the point is that only the METHOD lost capability, never the
    /// route.
    /// <para>
    /// Confined to routes under the run's own <c>ApiRepoPathPrefix</c>, because that is the whole set this
    /// policy permits on <c>main</c> today. Off-repo read routes (CI policy evaluations, builds, the
    /// org-scoped project route) are denied here by the pre-existing route scope, not by this change, and
    /// asserting on them would pin a capability that does not exist yet.
    /// </para>
    /// </summary>
    [Theory]
    ...
    /// <summary>
    /// The safe default. This is the shape the defect actually had: <c>PolicyEnforcedHttpClientFactory</c>
    /// called <c>BuildForRun</c> and simply said nothing about write capability, and the parameter's
    /// <c>true</c> default handed every provider client the ability to POST on a collect-only run — with no
    /// line of code anywhere reading as a grant. Passing the flag at that one call site fixes that site; this
    /// asserts the property that stops the next one being written, so a construction site added tomorrow is
    /// collect-only until someone asks for a write capability by name.
    /// </summary>
    [Fact]
    public void A_policy_built_with_no_explicit_grant_cannot_post_a_review_comment() =>
        AssertNoWriteCapabilityWithoutAnExplicitGrant(SandboxOperation.PostReviewComment, "POST");
...
public sealed class ReviewSpawnGateTests : LoggingTestBase
{
    ...
    public void PostingEnabled_allows_the_same_posting_capable_template()
    {
        ...
        logs.Capturing.CountAtLevel(LogLevel.Error, "REFUSED").Should().Be(0);
        refusals.Recorded.Should().BeEmpty();
    }
