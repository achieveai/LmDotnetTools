...
public class MockHttpHandlerBuilderTests
{
    ...
    public void RequestMatcher_JsonElementMatching_ShouldWorkCorrectly()
    {
        ...
        Assert.True(RequestMatcher.MatchesRecordedRequest(element1, element2, true));

        // Same content should match flexibly
        Assert.True(RequestMatcher.MatchesRecordedRequest(element1, element2, false));

        // Different content should not match exactly
        Assert.False(RequestMatcher.MatchesRecordedRequest(element1, element3, true));

        // Different content should not match flexibly (different message content)
        Assert.False(RequestMatcher.MatchesRecordedRequest(element1, element3, false));
    }
    ...
    public async Task MiddlewareChain_ShouldExecuteInCorrectOrder()
    {
        ...
        var httpClient = new HttpClient(handler);
        var content = new StringContent("""{"test": "request"}""", Encoding.UTF8, "application/json");
