...
public class PublishLaunchScriptTests
{
    ...
    private static string CodeOnlyText => string.Join(
        '\n',
        ScriptText
            .Split('\n')
    ...
    public void Script_BuildsClientUnconditionally_NpmCiBeforeNpmRunBuild()
    {
        ...
        ScriptText.Should().Contain("npm ci --prefix $ClientAppDirectory", "npm ci must run unconditionally in Invoke-ClientBuild");
        ScriptText.Should().Contain("npm run build --prefix $ClientAppDirectory", "npm run build must run unconditionally in Invoke-ClientBuild");
        ScriptText.Should().NotContain("SkipClientBuild", "the unconditional standalone build must not have a skip-the-client-build escape hatch");
        ScriptText.Should().NotContain("Test-ClientDependenciesFresh", "there is no freshness-shortcut check in the unconditional standalone pipeline");
    ...
    public void Script_PublishesWithBuildClientAppFalseAndNoNoBuildFlag()
    {
        ...
        publishLine.Should().Contain("'-p:BuildClientApp=false'", "publish must pass BuildClientApp=false so MSBuild does not duplicate the client build");
        ...
        publishLine.Should().NotContain("--no-build", "the live invocation must not pass --no-build -- publish must build the server itself");
    }
    ...
    public void Script_CreatesRunDirectoryUnderRepositoryRootScratchpad()
    {
        ...
        ScriptText.Should().Contain(
            "function New-PublishRunDirectory",
            "a dedicated helper must resolve the run directory");
        ScriptText.Should().Contain(
    ...
    public void Script_ValidatesFullArtifactAfterPublishAndBeforeLaunch()
    {
        ...
        publishCallIndex.Should().BeLessThan(validateCallIndex, "the artifact must be published before it is validated");
        validateCallIndex.Should().BeLessThan(startCallIndex, "the artifact must be validated before the published exe is launched");
    }
    ...
    public void Script_ValidatesJsCssAssetReferencesWithWwwrootBoundaryContainment()
    {
        ...
        ScriptText.Should().Contain("function Get-PublishedAssetReferences", "a dedicated helper must extract JS/CSS asset references from index.html");
        ScriptText.Should().Contain(
            @"'(?:src|href)=[""''](?<path>/dist/[^""''?#]+\.(?:js|css))(?:[?#][^""'']*)?[""'']'",
            "the asset-reference regex must match local /dist/*.js and *.css references");
        ScriptText.Should().Contain("function Confirm-PublishArtifact", "a dedicated helper must validate the full publish artifact");
        ScriptText.Should().Contain(
    ...
    public void Script_LaunchesPublishedExecutableViaProcessStartInfo_NeverSourceOrStartProcess()
    {
        ...
        ScriptText.Should().Contain("$startInfo = [System.Diagnostics.ProcessStartInfo]::new()", "the backend must be launched via a direct ProcessStartInfo");
        ScriptText.Should().Contain("$startInfo.FileName = $artifact.ExecutablePath", "the FileName must be the validated published executable, not the source project");
        ...
        ScriptText.Should().NotContain("Start-Process", "the script must not launch any child via Start-Process (the old wrapper-hop mechanism)");
    ...
    public void Script_SetsProductionEnvironmentOnlyOnChildProcess_NeverParentSession()
    {
        ...
        ScriptText.Should().NotContain("$env:ASPNETCORE_ENVIRONMENT", "Production must not be set on the parent PowerShell session's environment");
        ScriptText.Should().NotContain("$env:DOTNET_ENVIRONMENT", "Production must not be set on the parent PowerShell session's environment");
    }
    ...
    public void Script_SetsExplicitEnvFileOverrideOnlyOnChildProcess_PointingAtProjectDirNotPublishDirectory()
    {
        ...
        ScriptText.Should().NotContain("$env:LMSTREAMING_ENV_FILE", "the env-file override must not be set on the parent PowerShell session's environment");
        ScriptText.Should().NotContain(
    ...
    public void Script_ContainsNoViteRuntimeConstructs()
    {
        ...
        ScriptText.Should().NotContain("npm run dev", "the client is built for production (npm run build), never run in dev mode");
        ScriptText.Should().NotContain("Test-ViteProxyReachesBackend", "there is no Vite-proxy readiness check in the standalone pipeline");
        ScriptText.Should().NotContain("Test-ViteEntryModuleResolves", "there is no Vite entry-module readiness check in the standalone pipeline");
        ScriptText.Should().NotContain("Get-Command npm.cmd", "the script must not resolve npm.cmd on PATH to launch a dev server");
    ...
    public void Script_CleansUpOnlyWithImmutablePositivePid()
    {
        ...
        ScriptText.Should().Contain("$backendProcessId = 0", "the PID must default to 0 before Start() is attempted");
        ScriptText.Should().Contain(
            "$backendProcessId = $backendProcess.Id",
            "the PID must be captured immediately after a successful Start()");

        var occurrencesOfIdCapture = System.Text.RegularExpressions.Regex.Matches(
            ScriptText, @"\$backendProcessId\s*=\s*\$backendProcess\.Id").Count;
        ...
        ScriptText.Should().NotContain("Stop-OwnedProcessTree -ProcessId $backendProcess.Id", "cleanup must never re-read .Id from the Process object");
        ScriptText.Should().NotContain("$null -ne $backendProcess", "the finally gate must not be a bare non-null Process check (true even when Start() failed)");
    }
    ...
    public void Script_SupervisesViaBoundedInterruptibleLoop_NotIndefiniteWaitForExit()
    {
        ...
        ScriptText.Should().Contain("while (-not $backendProcess.WaitForExit(250)) { }", "supervision must poll in short bounded increments, not block indefinitely");
        ...
        CodeOnlyText.Should().NotContain("CancelKeyPress", "no global Ctrl+C handler may be registered in live code");
        CodeOnlyText.Should().NotContain("WaitForExit()", "no live call to the indefinite, no-argument WaitForExit() may remain");
    }
    ...
    public void Script_RunStateRecordsPublishDirectoryAndOmitsViteObject()
    {
        ...
        ScriptText.Should().MatchRegex(@"publishDirectory\s*=\s*\$publishDirectory", "run-state must record the resolved publish directory");
        ScriptText.Should().MatchRegex(@"backend\s*=\s*\[pscustomobject\]@\{", "run-state must record a single backend object");
        ScriptText.Should().Contain(
        ...
        ScriptText.Should().NotMatchRegex(@"(?m)^\s*vitePort\s*=", "the run-state object must not have a vitePort property");
    }
    ...
    public void Script_PublishDirectoryIsRetainedNotDeletedAfterRun()
    {
        ...
        originalPipelineText.Should().NotContain("Remove-Item", "the publish directory/run-state must never be deleted by this script");
        ScriptText.Should().NotContain("Remove-RunStateFile", "there is no run-state cleanup helper -- the artifact is retained");
        ScriptText.Should().NotContain(
    ...
    public void Script_RetainsPortForceAndWebhookWorkspaceConfigurationBehavior()
    {
        ...
        // standalone launcher.
        ScriptText.Should().Contain("Port resolution rules:", "the -Port parameter help must retain the resolution rules");
        ScriptText.Should().Contain(
        ...
        ScriptText.Should().Contain("[string] $WebhookBaseUrl = 'https://lmstreaming.bhakars.internal'", "the -WebhookBaseUrl default must be retained");
        ScriptText.Should().Contain("'--SandboxGateway:WorkspaceBasePath='", "the workspace base path must still be emptied on the command line for the remote gateway");
        ScriptText.Should().Contain("\"--Auth:Webhook:PublicBaseUrl=$WebhookBaseUrl\"", "the resolved webhook base URL must still be passed on the command line");
    }
    ...
    public void Script_TestPortFreeChecksBothIPv4AndIPv6FamiliesForTheBackendPort()
    {
        ...
        ScriptText.Should().Contain("[System.Net.IPAddress]::IPv6Any", "the backend port check must include the IPv6 wildcard family");
        ScriptText.Should().Contain("[System.Net.IPAddress]::Loopback");
        ScriptText.Should().Contain("[System.Net.IPAddress]::IPv6Loopback", "the backend port check must include the IPv6 loopback family");
        ScriptText.Should().Contain(
    ...
    public void Script_IsDotSourceableWithoutRunningInvokeMain()
    {
        ...
        ScriptText.Should().Contain("function Invoke-Main {", "the default pipeline body must be wrapped in a named Invoke-Main function");
        ScriptText.Should().MatchRegex(
        ...
        invokeMainDefinitionIndex.Should().BeLessThan(guardIndex, "Invoke-Main must be fully defined before the trailing guard decides whether to call it");
    }
    ...
    public void Script_DestinationModeBranchesBeforePortResolutionAndNeverLaunches()
    {
        ...
        var destinationBranchIndex = ScriptText.IndexOf("Invoke-DestinationPublish -ProjectFile $ProjectFile", StringComparison.Ordinal);
        var portResolutionIndex = ScriptText.IndexOf("Write-Phase 'Resolve port'", StringComparison.Ordinal);
        destinationBranchIndex.Should().BeGreaterThan(-1, "Invoke-Main must call Invoke-DestinationPublish when -DestinationDirectory is provided");
        portResolutionIndex.Should().BeGreaterThan(-1);
        destinationBranchIndex.Should().BeLessThan(portResolutionIndex, "the destination-mode branch must be checked before port resolution ever runs");
        ...
        destinationFunctionsRegion.Should().NotContain("ProcessStartInfo", "destination-mode functions must never launch the published executable");
        destinationFunctionsRegion.Should().NotContain("Resolve-InstancePort", "destination-mode functions must never resolve a port");
        destinationFunctionsRegion.Should().NotContain("Wait-HttpReady", "destination-mode functions must never perform readiness polling");
        destinationFunctionsRegion.Should().NotContain("Write-RunStateFile", "destination-mode functions must never write a .run-state.json");
    }
    ...
    public void Script_PreserveListConstantMatchesSpecExactly()
    {
        ...
        normalizedScriptText.Should().Contain(
