...
public class McpToolInvocationTests
{
  ...
  public async Task GreetingTool_SayHello_ReturnsGreetingWithName()
  {
    ...
    // Test regular name
    var regularResult = await functionMap["SayHello"]("{\"name\":\"John\"}");
    ...
    var specialCharsResult = await functionMap["SayHello"]("{\"name\":\"Maria O'Connor-Smith\"}");
    ...
    // Test with emoji
    var emojiResult = await functionMap["SayHello"]("{\"name\":\"User 😀\"}");
    ...
    var longNameResult = await functionMap["SayHello"]($"{{\"name\":\"{longName}\"}}");
  ...
  public async Task GreetingTool_SayGoodbye_ReturnsGoodbyeMessage()
  {
    ...
    // Act
    var result = await functionMap["SayGoodbye"]("{\"name\":\"John\"}");
  ...
  public async Task CalculatorTool_CalculatesMultipleOperations()
  {
    ...
    // Test multiplication
    var multiplyResult = await functionMap["Multiply"]("{\"a\":4,\"b\":5}");
    double multiplyValue = double.Parse(multiplyResult.Trim('\"'));
    Assert.Equal(20, multiplyValue);
    
    // Test subtraction
    var subtractResult = await functionMap["Subtract"]("{\"a\":10,\"b\":3}");
    ...
    // Test square root
    var sqrtResult = await functionMap["Sqrt"]("{\"x\":16}");
    double sqrtValue = double.Parse(sqrtResult.Trim('\"'));
    Assert.Equal(4, sqrtValue);
    
    // Test power
    var powerResult = await functionMap["Power"]("{\"x\":2,\"y\":3}");
  ...
  public async Task CalculatorTool_HandlesErrorsGracefully()
  {
    ...
    var sqrtNegativeResult = await functionMap["Sqrt"]("{\"x\":-1}");
    // Check that the result contains error information - simpler assertion that doesn't rely on exact format
