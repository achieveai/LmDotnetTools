...
/// any value this build does not recognize</b>, blocks it. That asymmetry is the whole point: the
/// rest of this contract preserves unknown discriminators and forwards them, but "preserve and
/// ignore" applied to an authorization decision is indistinguishable from "allow". Approval is the
/// one place where an unknown value fails closed.
/// </para>
/// <para>
/// Codes divide into two groups. <see cref="Allowed"/> and <see cref="Denied"/> are the only ones
/// an approver may submit; the rest are derived by the host from what actually happened and are
/// rejected if they arrive from a remote decision endpoint. Use
/// <see cref="IsApproverSubmittable"/> to make that distinction.
/// </para>
/// </remarks>
public static class ToolApprovalOutcomes
{
    ...
    /// wait, any provider or host deadline, run or turn cancellation, a provider interrupt,
    /// shutdown, and revocation.
    /// </summary>
    public const string Timeout = "timeout";

    /// <summary>Too many approvals were already pending for this owner or process.</summary>
    public const string Overload = "overload";

    /// <summary>
    /// A previously granted approval was withdrawn before the handler was invoked.
    /// </summary>
    ...
    /// </summary>
    /// <param name="outcome">The decision code received from an approver.</param>
    /// <returns>
    /// <see langword="true"/> for <see cref="Allowed"/> and <see cref="Denied"/> only.
    /// </returns>
    /// <remarks>
    /// The remaining codes describe what the host observed — a timeout, an overload, a revocation.
    /// Accepting them from a remote peer would let that peer assert an outcome it cannot know, so
    /// they are refused at the decision endpoint.
    /// </remarks>
    public static bool IsApproverSubmittable(string? outcome) =>
        string.Equals(outcome, Allowed, StringComparison.Ordinal)
        || string.Equals(outcome, Denied, StringComparison.Ordinal);
}
