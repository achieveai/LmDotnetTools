...
public class AgentController : ControllerBase
{
    ...
    {
        _logger.LogInformation("RunAgent called with agent: {AgentName}, message: {Message}, stream: {Stream}",
            request.AgentName, request.Message, request.Stream);

        if (!ModelState.IsValid)
        {
            _logger.LogWarning("Invalid request model state");
            return BadRequest(new RunAgentResponse
            {
                Status = "error",
        ...
        try
        {
            ...
            if (agent == null)
            {
                ...
                {
                    Status = "error",
                    ErrorMessage = $"Unknown agent: {request.AgentName}. Available agents: ToolCallingAgent, InstructionChainAgent",
                    AgentName = request.AgentName
                });
            }

            // Create or use existing session
            _logger.LogInformation("[DEBUG] Received conversationId from request: {ConversationId}", request.ConversationId ?? "NULL");
            var sessionId = request.ConversationId ?? Guid.NewGuid().ToString();
            _logger.LogInformation("[DEBUG] Using session ID: {SessionId} for agent: {AgentName} (Generated new: {Generated})",
                sessionId, request.AgentName, request.ConversationId == null);
            ...
            await _eventPublisher.PublishAsync(new RunStartedEvent
            {
                SessionId = sessionId,
            ...
            _ = Task.Run(async () =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    await _eventPublisher.PublishAsync(new ErrorEvent
                    {
                        SessionId = sessionId,
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new RunAgentResponse
            {
                Status = "error",
    ...
    public ActionResult<object> Health()
    {
        ...
        return Ok(new
        {
            status = "healthy",
            timestamp = DateTime.UtcNow,
            agents = new[] { "ToolCallingAgent", "InstructionChainAgent" }
        });
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await foreach (var message in responseStream.WithCancellation(cancellationToken))
            {
                ...
                {
                    _logger.LogDebug("Publishing event {EventType} for session {SessionId}",
                        evt.Type, sessionId);
            ...
            await _eventPublisher.PublishAsync(new RunFinishedEvent
            {
                SessionId = sessionId,
