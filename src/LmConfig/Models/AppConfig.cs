...
public record AppConfig
{
    ...
    {
        Debug.WriteLine($"DEBUG: AppConfig.GetModel called with modelId: {modelId ?? "NULL"}");
        Debug.WriteLine($"DEBUG: Models collection is null: {Models == null}");
        if (Models != null && !string.IsNullOrEmpty(modelId))
        {
            Debug.WriteLine($"DEBUG: Models collection count: {Models.Count}");
    ...
    {
        Debug.WriteLine($"DEBUG: AppConfig.GetModelsWithCapability called with capability: {capability ?? "NULL"}");
        Debug.WriteLine($"DEBUG: Models collection is null: {Models == null}");
        ...
        if (Models != null && !string.IsNullOrEmpty(capability))
        {
            ...
            Debug.WriteLine($"DEBUG: Found {matchingModels.Count} models with capability {capability}");
            foreach (var model in matchingModels)
            {
                Debug.WriteLine($"DEBUG: Model {model.Id} has capability {capability}");
            }
    ...
    {
        Debug.WriteLine($"DEBUG: GetProviderConnection called with: '{providerName}'");
        Debug.WriteLine($"DEBUG: ProviderRegistry is null: {ProviderRegistry == null}");

        if (ProviderRegistry == null || string.IsNullOrWhiteSpace(providerName))
        {
            Debug.WriteLine("DEBUG: Returning null - registry null or provider name empty");
            return null;
        }

        Debug.WriteLine($"DEBUG: ProviderRegistry count: {ProviderRegistry.Count}");
        Debug.WriteLine($"DEBUG: Available providers: {string.Join(", ", ProviderRegistry.Keys)}");

        // Try exact match first (case-sensitive)
        if (ProviderRegistry.TryGetValue(providerName, out var provider))
        {
            Debug.WriteLine($"DEBUG: Found exact match for '{providerName}'");
            return provider;
        }

        Debug.WriteLine("DEBUG: No exact match, trying case-insensitive search");
        ...
        if (match.Value != null)
        {
            Debug.WriteLine($"DEBUG: Found case-insensitive match: '{match.Key}'");
        }
        else
        {
            Debug.WriteLine($"DEBUG: No match found for '{providerName}'");
        }
...
public record ProviderConnectionInfo
{
    ...
    public ValidationResult Validate()
    {
        ...
        if (string.IsNullOrWhiteSpace(apiKey))
        {
            ...
            {
                warnings.Add(
                    $"Environment variable '{ApiKeyEnvironmentVariable}' is not set. Will use Claude Code authentication if available."
                );
            }
            else
            {
