...
public record SubAgentOptions
{
    ...
    /// cheap orchestrator from stronger reviewers — and so it sits at a specific rung of the ladder:
    ...
    /// template is authored wherever its markdown lives — for the review host, in a workspace the operator
    /// does not edit and the daemon cannot read — so a template-declared model would otherwise silently
    /// override the one the operator configured and pays for. A per-spawn choice still wins, because that is
    /// the parent agent making a deliberate, task-specific decision at dispatch time.
    /// </para>
    /// <para>
    /// Null/blank (default) = no conversation default, so every existing consumer keeps the previous
    /// ordering exactly. When it does apply, the selection is reported as
    /// <c>conversation-default</c> in <see cref="SubAgentModelRouting.SelectionSource"/>, which is the
    /// only way an operator can tell "the configured model won" from "nothing was configured and the child
    /// inherited the parent" — the two states this knob existed to distinguish and previously could not.
    /// </para>
    /// </summary>
