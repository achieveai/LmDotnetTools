...
/// <summary>
/// Converts Codex event envelopes into LmCore message types.
/// Manages accumulator state for streaming delta/snapshot reconciliation.
/// </summary>
internal sealed class CodexEventTranslator
{
    ...
    public CodexEventTranslator(CodexSdkOptions options, ILogger? logger)
    {
    ...
    /// <summary>
    /// The thread ID for messages. Set by the agent loop.
    /// </summary>
    public string ThreadId { get; set; } = string.Empty;

    /// <summary>
    /// When a thread.started event is received, this is set to the extracted Codex thread ID.
    /// The caller should read this after each <see cref="ConvertEventToMessages"/> call.
    /// </summary>
    public string? LastExtractedCodexThreadId { get; internal set; }

    /// <summary>
    /// Resets per-run state. Should be called at the start of each run.
    /// </summary>
    ...
    public List<IMessage> ConvertEventToMessages(JsonElement eventElement, string runId, string generationId)
    {
        ...
        switch (eventType)
        {
            case "thread.started":
            case "thread/started":
                LastExtractedCodexThreadId = ExtractThreadId(eventElement, LastExtractedCodexThreadId);
                return [];
    ...
    internal List<IMessage> ConvertToolCallMessage(
        string eventType,
        JsonElement itemElement,
        string? itemId,
        string runId,
    ...
    #region Internal Tool Conversion
    ...
    public static string? ExtractReasoningText(JsonElement itemElement)
    {
    ...
    public static string BuildSummary(string itemType, JsonElement item)
    {
        return itemType switch
        {
    ...
    public static (string Delta, string Accumulated) AppendAndDiff(string existing, string current)
    {
    ...
    #region Internal Tool Payload Builders
    ...
    public static JsonElement BuildInternalToolResultPayload(
        string toolName,
        JsonElement itemElement,
        string source,
        string status)
