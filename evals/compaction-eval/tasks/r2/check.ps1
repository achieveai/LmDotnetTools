#!/usr/bin/env pwsh
# r2 checker: answers.json vs hidden key (normalised strings), plus a sources.md line per key.
param(
...
[ordered]@{ task = 'r2'; outcome = $outcome; score = [math]::Round($passed / $total, 4); checks = $checks } | ConvertTo-Json -Depth 5 | Set-Content -Path $Out -Encoding utf8
Write-Host "r2: $outcome ($passed/$total)"
exit 0
