#!/usr/bin/env pwsh
# c2 checker: hidden unittest suite against logstat.py (one check per test), plus README and the agent's own tests.
param(
...
$tests = @('test_status_counts', 'test_top_paths', 'test_errors_by_hour', 'test_slowest', 'test_help_exits_zero',
    'test_unknown_command_exits_two', 'test_missing_dir_exits_two', 'test_malformed_line_exits_two')
