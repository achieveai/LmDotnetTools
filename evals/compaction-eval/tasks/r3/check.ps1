#!/usr/bin/env pwsh
# r3 checker: answers.json vs hidden key (normalised strings), a sources.md line per key, and the embargoed pages 01-05 gone.
param(
...
$remaining = @(Get-ChildItem (Join-Path $Workspace 'kb') -File -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '^0[1-5]-.*\.md$' })
Add-Check 'embargoed pages 01-05 deleted' ($remaining.Count -eq 0) "still present: $($remaining.Name -join ', ')"

$passed = @($checks | Where-Object { $_.pass }).Count
$total = $checks.Count
$outcome = if ($passed -eq $total) { 'pass' } elseif ($passed -eq 0) { 'fail' } else { 'partial' }
[ordered]@{ task = 'r3'; outcome = $outcome; score = [math]::Round($passed / $total, 4); checks = $checks } | ConvertTo-Json -Depth 5 | Set-Content -Path $Out -Encoding utf8
Write-Host "r3: $outcome ($passed/$total)"
exit 0
