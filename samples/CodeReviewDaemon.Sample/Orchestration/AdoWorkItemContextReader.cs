...
internal sealed record AdoWorkItemContext
{
    ...
    /// as <c>AdoCiStatus.OmittedFailureMessages</c>: a cut list reads to the reviewer as the complete set.
    /// </summary>
...
/// itself, in code, before the reviewer is ever asked a question — the same move that made CI visible.
/// </para>
/// <para>
...
/// <see cref="SandboxOperation.ReadProviderMetadata"/>, exactly as <c>AdoCiStatusReader</c> does, so the same
...
/// escape would cost the PR its review — the trade <c>AdoCiStatusReader</c> documents and <c>AdoPrProvider</c>'s
/// metadata lookups already make. Unlike CI, though, a failed read is RENDERED rather than dropped: see
/// <see cref="AdoWorkItemLookup.Failed"/>.
/// </para>
/// </summary>
internal sealed class AdoWorkItemContextReader
{
    ...
    /// <c>AdoCiStatusReader</c> and <c>AdoPrProvider</c> use. The token is fetched per request so an expiry
