...
internal static class DaemonOperationPolicy
{
    ...
    /// <summary>
    /// ADO REST project route: <c>/{org}/_apis/projects/{project}</c> — org-scoped, so it cannot live under
    /// <see cref="AdoApiRepoPrefix"/>, which is why it needs naming separately (see
    /// <see cref="ReviewScope.ApiProjectMetadataPath"/>). <c>null</c> when the repo carries no project and
    /// there is consequently nothing to allow.
    /// </summary>
    private static string? AdoApiProjectPath(RepoIdentity repo) =>
        string.IsNullOrEmpty(repo.Project) ? null : $"/{repo.OrgOrOwner}/_apis/projects/{repo.Project}";

    /// <summary>
    /// The ADO REST route roots <c>AdoCiStatusReader</c> reads a PR's CI verdict from — project-scoped, so
    /// like <see cref="AdoApiProjectPath"/> they cannot live under <see cref="AdoApiRepoPrefix"/> (see
    /// <see cref="ReviewScope.ApiCiStatusPaths"/>). Empty when the repo carries no project, since without one
    /// there is no route to name.
    /// <para>
    /// Exactly three roots, one per call the reader makes. <c>build/builds</c> covers both the build object
    /// and its <c>/timeline</c> child because they share that root — <see cref="OperationPolicy"/> treats a
    /// root as a directory boundary, so <c>builds/39168345/timeline</c> is in and <c>builds-archive/1</c> is
    /// not. The per-assembly detail route (<c>_apis/test/runs</c>) is deliberately absent: the reader does
    /// not call it, and a route granted "in case" is a route nothing will notice going unused.
    /// </para>
    /// </summary>
    private static IReadOnlyList<string> AdoApiCiStatusPaths(RepoIdentity repo) =>
