...
internal sealed class HostGitCommandRunner : ISandboxCommandRunner
{
    ...
    /// </summary>
    private static readonly HashSet<string> S_remoteVerbs =
        new(StringComparer.Ordinal) { "fetch", "clone", "push", "pull", "ls-remote", "submodule" };

    /// <summary>Remote verbs that accept <c>--progress</c>. <c>ls-remote</c> talks to a remote but has no
    /// such flag, so it is announced without one.
    /// <para>
    ...
    /// (verified against a positive control). See <see cref="ProgressAnchorIndex"/>.
    /// </para>
    /// </summary>
    private static readonly HashSet<string> S_progressVerbs =
        new(StringComparer.Ordinal) { "fetch", "clone", "push", "pull", "submodule" };
    ...
    /// </summary>
    private static readonly HashSet<string> S_packWritingVerbs =
        new(StringComparer.Ordinal) { "fetch", "clone", "pull", "submodule" };
    ...
    /// commands stay bounded by <see cref="_maxDuration"/>, which is the honest bound for something that
    /// cannot be watched.
    /// </para>
    /// </summary>
    private static readonly HashSet<string> S_silentVerbs =
        new(StringComparer.Ordinal) { "submodule" };

    /// <summary>How often the watchdog re-checks the deadlines. Fine enough to kill promptly, coarse
    /// enough to cost nothing next to a multi-minute fetch.</summary>
    private static readonly TimeSpan S_watchdogTick = TimeSpan.FromMilliseconds(100);

    /// <summary>How often a still-running remote operation reports progress. An eleven-minute fetch
    /// produces roughly twenty lines — a bargain against the two wrong "it's hung" diagnoses the silence
    /// caused — while anything finishing inside one interval stays down to a start and a finish.</summary>
    ...
    public async Task<SandboxCommandResult> RunAsync(SandboxCommand command, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(command.WorkingDirectory) && !Directory.Exists(command.WorkingDirectory))
        {
            ...
            _logger.LogDebug("Host git '{Argv}' exited {Exit}: {Stderr}",
        ...
        var argv = isRemote && S_progressVerbs.Contains(verb!)
            ? WithProgress(command.Argv, verb!)
            : command.Argv;
        ...
        var preexistingTempPacks = verb is not null && S_packWritingVerbs.Contains(verb)
        ...
        try
        {
            ...
            captureComplete = await DrainWithinDeadlineAsync(stdout, stderr, timeoutCts).ConfigureAwait(false)
        ...
        {
            _logger.LogDebug("Host git '{Argv}' exited {Exit}: {Stderr}",
                string.Join(' ', argv), result.ExitCode, result.Stderr);
        }
    ...
    private async Task<string> ReadStreamAsync(
        StreamReader reader, ActivityClock activity, string? progressVerb, CancellationToken cancellationToken)
    {
    ...
    /// <summary>Cancels <paramref name="timeoutCts"/> once the command has been silent for longer than the
    /// idle timeout, or running for longer than the ceiling.</summary>
    ...
    /// <summary>
    /// Removes the pack files the killed command abandoned, and says how much that reclaimed. Reported at
    /// Warning rather than Debug because it is the audible half of the fix: a sweep that runs and says
    ...
    /// <summary>
    /// Waits for both readers to reach EOF, bounded by <see cref="S_drainDeadline"/>. Returns true only
    /// when both drained naturally, which is the sole condition under which the captured output is known
    /// to be the whole of what the command produced.
    /// </summary>
    /// <remarks>
    ...
    private static async Task<bool> DrainWithinDeadlineAsync(
        Task<string> stdout, Task<string> stderr, CancellationTokenSource timeoutCts)
    {
    ...
    /// <summary>Inserts <c>--progress</c> immediately after the verb, unless the caller already asked for
    /// it or explicitly suppressed it with <c>--no-progress</c>.</summary>
