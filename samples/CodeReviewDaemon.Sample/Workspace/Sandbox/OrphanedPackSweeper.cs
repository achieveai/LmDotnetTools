...
/// <c>index-pack</c> is writing leaves its <c>tmp_pack_*</c> behind. Nothing reclaims it inside any window
/// that matters: <c>git gc</c> only prunes stale temp files past <c>gc.pruneExpire</c>, which defaults to
/// TWO WEEKS, and the daemon never sets that key anywhere. On a repository whose packs run to 5.45 GB each,
/// every timeout-kill leaks multiple gigabytes permanently. Measured: 35 orphans totalling 245.35 GB in one
/// submodule's pack directory, which grew the WSL disk image until it filled the host volume and the
/// filesystem remounted read-only. A single surviving orphan was 2.98 GB on its own.
/// </para>
/// <para>
...
/// SAME object store, started after our snapshot. Nothing in this tree rules that out — there is no
/// per-store lock around the pack-writing verbs, and several call paths run against their own clone with no
/// coordination at all — so safety does not rest on one: a candidate is also required to be held open by NO
/// process. See <see cref="OpenTempPackPaths"/>. The two conditions are ANDed, so the guard can only ever
/// spare a file, never condemn one.
/// </para>
/// <para>
...
internal static class OrphanedPackSweeper
{
    ...
    /// <summary>
    /// Bounds the walk for nested submodule object stores. Submodule git directories nest one level per path
    /// segment (<c>modules/repos/Nova/modules/…</c>), so a handful of levels covers every real layout while a
    /// cycle through a symlink cannot run away.
    /// </summary>
    private const int MaxModuleDepth = 12;

    /// <summary>
    /// Where the open-descriptor evidence lives. A constant so the one test that has to prove the guard
    /// degrades safely on a platform without it has something to name.
    /// </summary>
    ...
    /// <summary>
    /// Whether a file name is one of the temp files <c>index-pack</c> writes. The single place the prefix
    /// rule lives, so the sweep and the open-handle scan cannot drift apart on what counts.
    /// </summary>
    ...
    /// <summary>
    /// Temp pack files directly inside one pack directory. Not recursive: git writes them beside the packs
    /// they will become, never below.
    /// </summary>
    ...
    /// <summary>
    /// The git directory itself, then every submodule git directory beneath its <c>modules</c> tree.
    /// Depth-bounded and pruned at <see cref="S_skipDirectories"/> so the 256-way loose-object fan-out is
    /// never entered.
    /// </summary>
