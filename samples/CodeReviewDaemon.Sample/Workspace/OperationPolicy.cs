...
internal sealed class OperationPolicy
{
    ...
    /// <para>
    /// The same flag now also carries the daemon's COLLECT-ONLY posture (<c>EnableCommentPosting</c>): a
    /// run the operator has not authorized to post gets <c>false</c> here, which is what turns "the daemon
    /// declines to call the publisher" into "the daemon cannot make a provider write at all". The two
    /// meanings coincide deliberately — both are "this policy has no write capability" — so there is one
    /// switch to reason about rather than two that can disagree.
    /// </para>
    /// </param>
