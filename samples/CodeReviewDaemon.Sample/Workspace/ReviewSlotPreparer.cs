...
internal sealed class ReviewSlotPreparer : IReviewSlotPreparer
{
    ...
    /// only hands the next one a fresh target. It is raised as <see cref="SlotHostPathRefusedException"/>, the
    /// type that means exactly this, and NOT as one of the two re-clone types: those drive the recovery ladder
    /// into a wipe, and this wipe is what already refused. An earlier note here defended throwing an untyped
    /// <see cref="InvalidOperationException"/> on that same "not one of the re-clone types" reasoning, which
    /// mistook the absence of a wrong type for the absence of a right one and left a security refusal
    /// indistinguishable from a failed <c>git</c> invocation at every handler above.
    /// </para>
    /// <para>
    /// Being a distinguishable type is what lets the caller RETIRE the slot. The refusal is a property of the
    /// address, not of the attempt: nothing about the next lease makes a planted entry readable or contained,
    /// so a slot returned to the pool's free stack after one comes straight back out as the next index and
    /// refuses again, forever, on a store no run can ever prepare. <c>ReviewSlotPool.LeaseAsync</c> already
    /// retires an index for exactly this reason when the refusal happens at lease time; the entry that stops
    /// this walk is a DESCENDANT of the three paths that guard checks, so a later lease cannot see it and the
    /// pool only learns the address is spent if this refusal is legible on the way out.
    /// </para>
    /// <para>
    ...
    /// which is the one thing this walk's refusals must not look like, since the caller retires the slot on a
    /// refusal and merely retries on an error.
    /// </para>
    /// </summary>
