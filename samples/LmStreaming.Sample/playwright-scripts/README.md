...
| `compaction.mjs` | **Just-in-time compaction (#721).** Needs the host test profile from PromptExamples.md "Compaction (#721) UI tests" (18,000-token window; the hard turn is `HARD_TURN` = 5). Set `HOST_MODE` to the host's mode. `Compact`: M2 — turn 5 decides `compact/hard`, state `Active`; exactly one `Context compacted` divider appears live (while the turn still runs), after the 5th user bubble, naming the checkpoint; it expands to goal + narrative; the panel reads `Compacted`; a `?threadId=` reload and the live turn after it still show one divider (next decision `skipped/cooldown`). M3 — that live turn 6 is the recall: `RecallConversation` returns the seed row (seq 1) from behind the boundary. M4 — with no JSON summary, turn 5 is `failed/summary_call_failed`, no divider, and turn 7 over the usable window gets no reply. `Shadow`: M1 — `shadow/hard` decisions, `Rejected/shadow`, raw requests keep growing, no divider. **Manual compaction** (Compact mode, `RUN_MANUAL`; needs `manualCompaction` in `/api/conversations/capabilities`): M5 — below the thresholds, **Compact now** opens a focus form; Enter POSTs `{"focus":…}` → 202; a MutationObserver catches `Compacting…` with the button disabled; the report reaches `Active`; one live divider whose body shows `## Focus` (also after reload); a second empty request → `409 nothing_to_compact` (the rows left are the kept tail) → "Nothing to compact yet." with the button enabled; switching conversation clears the status line. M6 — at 400px the header button and open form fit, with no sideways scroll. `RUN_AUTO=false` runs M5–M6 alone. Screenshots to `.logs/manual/pw721-*.png` (M1–M4) and `compaction-m5-*.png` / `compaction-m6-*.png`. BASE `:5173`. |
| `mode-switch-keeps-subagent.mjs` | **A mode/model switch on the API arm is served ON the live loop, so running sub-agents survive.** Background-spawns a child parked on a 10m timer `Wait`, then switches mode and switches provider (`test-anthropic` → `test`, both API-arm) while idle, asserting after each that `/api/conversations/{id}/subagents` still lists the **same `agentId`** as Running — and that no row was ever stamped `host_shutdown` (disposal's stamp, which a recovery re-listing would otherwise hide). Crossing onto a CLI arm still recreates, by design. |
