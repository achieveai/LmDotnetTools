...
///         entry only costs the next poll for that thread one extra rescan — it does not lose data. Only
///         REAL recorded rosters ever occupy a slot (see the forget-epoch design below) — the bounded
///         structure never has to make room for bookkeeping that carries no roster of its own.
///     </para>
///     <para>
...
///     </para>
///     <para>
///         <b>Scan/delete interleaving — cache-wide forget epoch (PR #245 stress review — a tombstone-based,
///         per-thread generation protocol reopens the exact resurrection it was meant to close).</b>
///         <see cref="Forget"/> above closes the gap for a scan that STARTS after the delete. It does not
///         by itself close the reverse ordering: a scan already in flight when <see cref="Forget"/> runs is
///         not cancelled by it — <c>AgentHierarchyService</c> still holds a completed, in-hand roster from
///         BEFORE the delete and, without a caller-provided sequencing signal, would call
///         <see cref="RecordRecovered"/> with it right after <see cref="Forget"/> returns, silently
///         re-inserting the deleted thread's stale roster.
///     </para>
///     <para>
///         An earlier version of this guard recorded a per-thread tombstone (a fourth <c>CacheEntry</c>
///         under a private sentinel owner) carrying its own generation counter, aging out under the SAME
///         bounded capacity structure as a real entry. That was the bug: because the tombstone occupied an
///         ordinary slot, ordinary capacity pressure (trivially reachable at <c>capacity: 1</c>, where a
///         SINGLE unrelated write evicts it) could evict the tombstone before the in-flight scan's stale
///         writeback landed. Evicting the tombstone reset that thread's per-thread counter back to its 0
///         baseline — the SAME value the stale scan had captured before the delete (for a thread whose
///         tombstone was its first-ever entry) — so the "generation still matches" check silently passed
///         and the deleted roster was resurrected anyway. A classic ABA hazard: the counter returned to a
///         value it had already held, and the guard could not tell the difference.
///     </para>
///     <para>
///         The fix removes the per-thread counter (and the tombstone entry) entirely in favor of one
///         cache-wide, monotonically increasing <see cref="_writeEpoch"/>, guarded by the same
///         <see cref="_gate"/> as everything else. <see cref="CaptureWriteEpoch"/> reads it BEFORE a scan
///         starts; <see cref="Forget"/> increments it (there is exactly one counter, so there is nothing
///         left for capacity eviction to reset — a deleted thread's slot is removed outright, not replaced
///         with a stand-in); <see cref="RecordRecovered"/> requires the caller to pass back the epoch it
///         captured and rejects the write outright if the CURRENT epoch has since moved on, regardless of
///         which thread's <see cref="Forget"/> caused that — <b>any</b> delete anywhere in the cache
///         invalidates every write captured before it, not just the one for the same thread id. This is
///         deliberately conservative: a scan for thread B in flight when thread A is deleted pays for one
///         extra, harmless rescan it did not strictly need, in exchange for a guard that can never wrap
///         back around to a value a stale caller already captured. Because eviction no longer has a
///         tombstone to reset, this holds true no matter how small <c>_capacity</c> is — including
///         <c>capacity: 1</c>, the case that broke the old per-thread design.
///     </para>
///     <para>
///         There is no default value for the <c>epoch</c> parameter <see cref="RecordRecovered"/> takes:
///         the predecessor's <c>generation = 0</c> default let a caller silently skip the
///         <see cref="CaptureWriteEpoch"/>/<see cref="RecordRecovered"/> pairing altogether (compiling
///         "successfully" straight into an unguarded write) — a footgun this redesign removes by making
///         every call site name the epoch it captured, explicitly, at the point it started scanning.
///     </para>
///     <para>
///         <see cref="_writeEpoch"/> is a <see cref="ulong"/> incremented with unchecked wraparound. A wrap
///         back to a value a caller already captured would need exactly 2^64 intervening <see cref="Forget"/>
///         calls between that caller's <see cref="CaptureWriteEpoch"/> and its <see cref="RecordRecovered"/> —
///         even at one billion deletes per second that is well over 500 years, so it is treated as
///         impossible in practice rather than guarded against explicitly.
///     </para>
/// </remarks>
public sealed class SubAgentScanCoverageCache
{
    ...
    /// <summary>Guards the collections below AND <see cref="_writeEpoch"/>; operations are in-memory
    /// dictionary/list/counter bookkeeping only, never held across the store scan itself, so contention
    /// is negligible.</summary>
    ...
    /// <summary>
    ///     Cache-wide monotonic counter; see the forget-epoch remarks above. Incremented by every
    ///     <see cref="Forget"/> call, regardless of thread id — never by <see cref="RecordRecovered"/> or
    ///     capacity eviction, and never reset.
    /// </summary>
    private ulong _writeEpoch;
    ...
    ///     either the scan has never completed for this thread, it last completed for a DIFFERENT owner
    ///     (the live manager was reset since), or the entry was removed by <see cref="Forget"/>.
    /// </summary>
    ...
    /// <summary>
    ///     Returns the cache-wide write epoch — see the forget-epoch remarks above. Call this BEFORE
    ///     starting a persisted-store scan and pass the result back into the matching
    ///     <see cref="RecordRecovered"/> call once the scan completes: this is what lets a
    ///     <see cref="Forget"/> (for this thread OR any other) that lands while the scan is in flight
    ///     cause that scan's writeback to be rejected instead of resurrecting stale data.
    /// </summary>
    public ulong CaptureWriteEpoch()
    {
        lock (_gate)
        {
            return _writeEpoch;
    ...
    ///     <paramref name="owner"/>, so no later call for the same (thread, owner) pair pays for the scan
    ...
    /// <param name="epoch">
    ///     The value <see cref="CaptureWriteEpoch"/> returned immediately BEFORE the scan that produced
    ///     <paramref name="rows"/> was started. There is deliberately no default: every caller must name
    ///     the epoch it captured, so an unguarded write can never compile by omission (see the remarks
    ///     above). If the cache-wide epoch has since moved on — a <see cref="Forget"/> ran for ANY thread
    ///     after the caller captured this value — the write is rejected and this method returns
    ///     <see langword="false"/> without touching the cache.
    /// </param>
    /// <returns>
    ///     <see langword="true"/> if the roster was recorded; <see langword="false"/> if the write was
    ///     rejected because the epoch captured before the scan is stale.
    /// </returns>
        ...
        ulong epoch)
    {
        lock (_gate)
        {
            ...
            var node = _writeOrder.AddLast(
                new KeyValuePair<string, CacheEntry>(threadId, new CacheEntry(owner, rows)));
    ...
    /// <summary>
    ///     Removes any recorded entry for <paramref name="threadId"/> outright, regardless of owner, so
    ///     <see cref="TryGetRecovered"/> misses unconditionally afterward, AND bumps the cache-wide write
    ///     epoch so any <see cref="RecordRecovered"/> already in flight (for this thread OR any other) with
    ///     an epoch captured before this call is rejected. Called on conversation delete (see remarks
    ///     above) so a later reuse of the same thread id never resurrects a deleted conversation's
    ///     recovered rows.
    /// </summary>
    /// <remarks>
    ///     Unlike the tombstone-based predecessor, this does not leave anything behind in
    ///     <see cref="_byThread"/>/<see cref="_writeOrder"/> — the slot is freed immediately, so it never
    ///     competes for capacity and can never be evicted out from under the epoch check (see the
    ///     forget-epoch remarks above for why that mattered).
    /// </remarks>
